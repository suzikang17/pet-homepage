...
- Test/build target is the **iPhone 16** simulator. `name=iPhone 16` alone is **ambiguous** on this machine (multiple OS versions installed) and xcodebuild rejects it — pin the OS: `-destination 'platform=iOS Simulator,name=iPhone 16,OS=18.2'` (or use the device UDID).
- Core Data model `contents`: use `codeGenerationType="none"` (Xcode 16/17 rejects the literal `"manual"` with a DVTAssertions failure; `"none"` IS the Manual/None setting).
...
- All test/build runs use the simulator: `-destination 'platform=iOS Simulator,name=iPhone 16,OS=18.2'`.
